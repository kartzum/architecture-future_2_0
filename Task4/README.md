...
![auto.drawio.png](auto.drawio.png)

[auto.drawio](auto.drawio)

### Обоснование конфигурации

Конфигурация автоматизирует создание IaaS‑контуров (сеть + VM + дополнительный диск) под среду разработку/тестирования.
Для простоты реализован базовый функционал и базовая конфигурация.
...
* Yandex Cloud выбран как публичный облачный провайдер, соответствующий законодательству РФ и предлагающий широкий спектр услуг. 
* Terraform-провайдер для Yandex Cloud официально поддерживается и регулярно обновляется.
* Использование аутентификации через OAuth-токен (переменная yc_token) упрощает первоначальную настройку, но в production рекомендуется перейти на сервисный аккаунт с ключом.
* Параметры cloud_id и folder_id позволяют точно указать целевое размещение ресурсов в иерархии облака, что критично для многопользовательских сред.

**Сеть**

Ресурсы.

* yandex_vpc_network с именем.
* yandex_vpc_subnet с CIDR 10.0.1.0/24 в зоне ru-central1-a.

Обоснование.

* VPC обеспечивает изолированное сетевое пространство, необходимое для безопасного взаимодействия ресурсов. 
* Создание отдельной сети позволяет изолировать проект от прочих ресурсов каталога. 
* Подсеть привязана к зоне доступности ru-central1-a — как к наиболее типовой и доступной. 
* Диапазон 10.0.1.0/24 (256 адресов) достаточен для небольшого проекта и не конфликтует с распространёнными корпоративными CIDR. 
* Имя подсети формируется от имени ВМ, что упрощает понимание, к какому ресурсу она относится.

**Виртуальная машина**

Ресурсы.

* Ресурсы: 2 vCPU, 4 GB RAM (переопределяются через переменные). Эти значения выбраны как минимально достаточные для типичных нагрузок. Но в конфигурационных файлах представлены минимальные значения.
* Загрузочный диск: 20 GB HDD, образ Ubuntu из семейства ubuntu-2204-lts. Но в конфигурационных файлах представлены минимальные значения.
* Подключение дополнительного диска: через secondary_disk.
* Метаданные: передача публичного SSH-ключа для доступа по ssh.

Обоснование.

* Платформа standard-v3 даёт современные CPU, что актуально для большинства задач. По необходимости её можно заменить на более бюджетную standard-v2 или более мощную версию с GPU. 
* Ресурсы 2 vCPU / 4 ГБ RAM — оптимальный баланс для тестовых и небольших продуктовых сред. Благодаря использованию переменных конфигурацию легко адаптировать под конкретные нужды.
* Образ из семейства ubuntu-2204-lts гарантирует актуальную версию ОС с обновлениями безопасности. 
* SSH-ключ — стандартный и надёжный способ доступа к Linux-ВМ.

**Диск**

Ресурсы.

* Тип: network-hdd – стандартный недорогой диск.
* Размер: задаётся переменной disk_size.
* Имя: привязано к имени ВМ.

Обоснование.

* Загрузочный диск создаётся автоматически внутри ресурса ВМ. Дополнительный диск нужен для отделения данных от ОС — это облегчает резервное копирование, миграцию и масштабирование. Например, диск с данными можно будет увеличить без пересоздания самой ВМ. Выбран тип network-hdd для экономии; при необходимости в переменных его можно сменить на network-ssd для более высокого IOPS.

**NAT и доступ**

Ресурсы.

* Параметр в network_interface: nat = true.

Обоснование.

* Включение NAT (Network Address Translation) автоматически назначает виртуальной машине публичный IP-адрес. В конфигурации используются локальные адреса для упрощения.

### Terraform-конфигурация и запуск

**Ручная подготовка**

* Аккаунт и платежный аккаунт в Yandex Cloud – регистрация и активация выполняются вручную.
* OAuth-токен – получается вручную.
* SSH-ключи – пара ключей генерируется вручную.
...
Выбор каждого параметра продиктован балансом простоты, экономичности и возможности масштабирования. 
За счёт использования переменных шаблон легко адаптируется под различные сценарии — все настройки при необходимости быстро изменяются.

Применяется декларативный подход, так как мы задаём желаемое состояние (сеть, подсеть, диск, ВМ с определёнными параметрами), а Terraform сам:

* определяет зависимости и порядок создания ресурсов;
* ведёт фактическое состояние в terraform.tfstate;
* вычисляет разницу между конфигурацией и реальностью, решая, что создать, изменить или удалить.

Ключевые преимущества перед ручным управлением:

* Предсказуемость — terraform plan показывает все будущие изменения.
* Идемпотентность — повторный apply не плодит лишние ресурсы.
* Поддерживаемость — вся конфигурация в Git, доступна история и code‑review.

**Последовательность запуска и примеры результатов**
